...
class FBLTests(unittest.TestCase):
    ...
    @staticmethod
    def inst_fbl():
        return i.FourBytesLiteral().set_eval('192.168.1.0')
    ...
    def test_exception_limited_list_too_big(self):
        li = i.LimitedList(5)
        self.assertRaises(Exception, lambda: i.FourBytesLiteral().set_eval(li))

    def test_exception_builtin_list_too_big(self):
        self.assertRaises(Exception, lambda: i.FourBytesLiteral().set_eval([192, 168, 1, 0, 5]))

    # Partial filling
    def test_partial_fill_limited_list(self):
        li = i.LimitedList(4).append_all([192, 168])
        inst = i.FourBytesLiteral().set_eval(li)

        self.assertEqual([192, 168, 0, 0], inst.bytes_list)

    def test_partial_fill_builtin_list(self):
        inst = i.FourBytesLiteral().set_eval([192, 168])
